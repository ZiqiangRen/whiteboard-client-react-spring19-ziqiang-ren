...
const LessonItem = ({lesson, selectLesson, editLesson, deleteLesson, titleChanged, index, selectedLessonIndex}) =>
    <li className={index === selectedLessonIndex? "nav-link active":"nav-link"}>
      ...
      <a className = "float-right" onClick={() => editLesson(lesson, titleChanged)}><i className="fa fa-edit"></i></a>
      <a className = "float-right" onClick={() => deleteLesson(lesson.id)}><i className="fa fa-trash"></i></a>
    </li>
